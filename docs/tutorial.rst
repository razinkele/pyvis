...
Integration with `Shiny for Python <https://shiny.posit.co/py/>`_
-----------------------------------------------------------------

Pyvis provides seamless integration with Shiny for Python, allowing you to create
interactive network visualizations in web applications with full bidirectional 
communication between Python and the vis.js network.

Basic Shiny Integration
~~~~~~~~~~~~~~~~~~~~~~~

The simplest way to embed a pyvis network in Shiny:
...
.. code-block:: python
    ...
    def server(input, output, session):
        @render.ui
        def my_network():
            net = Network(height="500px", width="100%", cdn_resources="remote")
...
For full event handling (clicks, selections, hovers), use the custom output binding:
...
.. code-block:: python
    ...
    def server(input, output, session):
        ...
        def network():
            ...
            net.add_node(1, label="Node 1", color="#e74c3c")
            net.add_node(2, label="Node 2", color="#3498db")
            net.add_edge(1, 2)
            return net
        ...
        def selected():
            ...
            if event:
                return f"Selected node: {event['nodeId']}"
            return "Click a node to select it"

    app = App(app_ui, server)

Available network events as Shiny inputs:
...
- ``input.{id}_doubleClick`` - Double-clicked
- ``input.{id}_hoverNode`` - Hovering over node
- ``input.{id}_dragEnd`` - Drag ended (includes new positions)
...
.. code-block:: python
    ...
    def server(input, output, session):
        ...
        def _():
            ctrl.select_nodes([1, 2, 3])  # Select specific nodes
        
        @reactive.effect
        @reactive.event(input.add_btn)
        def _():
            ctrl.add_node({"id": 100, "label": "New Node", "color": "#1abc9c"})

Available controller methods:

**Selection:** ``select_nodes()``, ``select_edges()``, ``unselect_all()``

**Viewport:** ``fit()``, ``focus(node_id)``, ``move_to()``

**Physics:** ``start_physics()``, ``stop_physics()``, ``stabilize()``

**Data:** ``add_node()``, ``update_node()``, ``remove_node()``, ``add_edge()``, ``remove_edge()``

**Clustering:** ``cluster_by_connection()``, ``cluster_by_hubsize()``, ``open_cluster()``

**Options:** ``set_options()``

For comprehensive documentation on Shiny integration, see the 
`Shiny Integration Guide <SHINY_INTEGRATION_GUIDE.md>`_.
