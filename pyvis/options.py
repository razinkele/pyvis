"""Options module for configuring network visualization settings.

This module provides classes for configuring various aspects of the network
visualization including physics simulations, layout options, edge rendering,
...
class _LegacyEdgeOptions(JSONSerializable):
    ...
    def inherit_colors(self, status: Union[bool, str]) -> None:
        """
    ...
    def toggle_smoothness(self, smooth_type: str) -> None:
        """
...
class Configure(JSONSerializable):
    """
    Handles the HTML part of the canvas and generates
    an interactive option editor with filtering.
    """

    def __init__(self, enabled: bool = False, filter_: Optional[Union[bool, List[str]]] = None):
...
class Layout(JSONSerializable):
    ...
    def set_separation(self, distance: int) -> None:
        """
        The distance between the different levels.
        """
        self.hierarchical.levelSeparation = distance

    def set_tree_spacing(self, distance: int) -> None:
        """
    ...
    def set_edge_minimization(self, status: bool) -> None:
        """
...
class Options(JSONSerializable):
    ...
    def __init__(self, layout: Optional[bool] = None):
        if layout:
            self.layout = Layout()
